...
export type FrontmatterSchema =
  | FrontmatterSchemaWithoutEffects
  | ZodEffects<FrontmatterSchemaWithoutEffects>;
...
export async function parse<TFrontmatterSchema extends FrontmatterSchema>(
  ...
  if (options?.frontmatterSchema) {
    const zodParseResults = await options.frontmatterSchema.safeParseAsync(
      frontmatterRawParseResults.frontmatterRaw,
    );
...
export function formatMarkdocError(error: ValidateError): string {
  ...
  const endLocation = error.location?.end
    ...
    : undefined;
  return `${startLocation ? `${formatErrorLocationRange(startLocation, endLocation)}: ` : ""}${
    error.error.message
  }`;
}
...
function formatErrorLocationRange(
  ...
  return startLocation
    ? `${formatErrorLocation(startLocation)}${
        endLocation ? `-${formatErrorLocation(endLocation)}` : ""
      }`
    : "";
}
...
export async function parseDir<TFrontmatterSchema extends FrontmatterSchema>(
  ...
  const fileMap = await walkDir(fsModule, dir, {
    ...
    fileFilter: (fileOptions) => {
      return getExtension(fileOptions.name) !== "mdoc"
        ? false
        : options?.fileFilter?.(fileOptions) ?? true;
    },
  });
...
export function formatParseDirResultErrors<TFrontmatterSchema extends FrontmatterSchema>(
  ...
  return flattenedFileList
    .map((item) =>
      item.data.isSuccessful ? "" : `${item.path}:\n${formatParseResultError(item.data)}`,
    )
    .filter(Boolean)
    .join("\n\n");
}
